...
| `data/spellterms.lua` | Bundled baseline `ns.bundledSpellTerms = { bySpec, byRace, byClass }` shipped with the addon — folded under each account's live scans by `rebuildMerged` so a fresh install has coverage without scanning. Generated by `/bmi export`. Optional (guarded). |
...
mergedSpellTerms     = { [fileID] = " names " }  -- in-memory union: bundled, then live stores
                                                 -- (whole blobs, never re-split per word)
...
rebuildMerged()           -- union ns.bundledSpellTerms then live spec+race+class into merged,
                          -- blob-whole via its own mergeBlob (never re-split on whitespace)
...
- **Stored terms are WHOLE spell names — never re-split a blob on whitespace.** `addName` appends each name entire, so a blob like `" final verdict templar's verdict "` (fid 461860) holds *two* names separated by single spaces — the same character that separates words *within* a name. The per-name boundaries are therefore unrecoverable from the blob alone, and a `gmatch("%S+")` re-split silently corrupts any icon whose names share a word: the second `verdict` hits the space-bounded dedup and `templar's verdict` collapses to `templar's`. **Both** blob consumers therefore union whole and compare by containment: `scan.lua`'s `rebuildMerged` (via its own `mergeBlob`) and `dataset.lua` (`mergeBlob` + `newNames`). Search is unaffected either way — it's plain substring matching — so a re-split only ever surfaces where names are *displayed* or *serialised*: it mis-reported `/bmi export` + `/bmi diff` (fixed in #32) and mangled the icon **tooltip** (the same bug left in `rebuildMerged`, fixed in #35). `spec/dataset_spec.lua` guards the tooling path and `spec/scan_spec.lua` the merged/tooltip path, including a cross-check that the two agree.
- **`mergeBlob` is deliberately duplicated in `scan.lua` and `dataset.lua`.** Both are independently removable modules, so neither may read the other's copy — a shared `ns.MergeSpellBlob` seam would make deleting `scan.lua` break `/bmi export` (the bundle still loads, so the merge path still runs). Both fold **bundled first, live second**, so a genuine partial overlap splices in the same order and the merged lookup matches what `/bmi export` would emit; `spec/scan_spec.lua`'s cross-check spec is what keeps them from drifting.
