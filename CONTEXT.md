...
| `core.lua` | **Setup file** (calls `LibNAddOn`, so it loads first). fileID→name map, picker registry (`PICKER_ADDONS`) + per-frame search state, search box injection, filter logic, multi-token matching, tooltips + right-click wiring. Exposes `ns.IconName` and `ns.refreshSearch`; reads the two optional term seams defensively. |
...
**TOC load order:** `icons.lua`, `core.lua`, `data/spellterms.lua`, `scan.lua`, `dataset.lua`, `aliases.lua`, `cleanup.lua`. `core.lua` is first because it calls `LibNAddOn(...)`; `scan.lua`/`dataset.lua`/`aliases.lua`/`cleanup.lua` make load-time `ns:register*` calls that need the wired namespace (`data/spellterms.lua` only sets a field, so its position is free). `dataset.lua` uses `ns.CanonRace` but only at runtime. All cross-module reads happen at runtime via `ns`, so beyond "setup first" the order is not otherwise significant.
