...
**Deps:** LibNAddOn (+ optional LibNUI) · **SavedVars:** `BetterMacroIconsDB` (account-wide, `X-NUI-DB`, DB v1) · **Commands:** `/bmi open`, `/bmi scan`, `/bmi coverage`, `/bmi export`, `/bmi debug`, `/bmi cleanup [preview]` · **UI:** raw WoW API (hooks Blizzard_MacroUI)
...
| `scan.lua` | **Removable module.** Spellbook scan → `ns.SpellTermsFor(fileID)`. Spec/race-keyed pooled store, folds in the `data/spellterms.lua` baseline, plus `/bmi coverage` (captured/missing) and `/bmi export` (dump the pooled terms as a new baseline). Delete this file + its `.toc` line and the addon still works (name search + aliases) without spell terms. |
| `data/spellterms.lua` | Bundled baseline `ns.bundledSpellTerms = { bySpec, byRace }` shipped with the addon — folded under each account's live scans by `rebuildMerged` so a fresh install has coverage without scanning. Generated by `/bmi export`. Optional (guarded); stub-empty until populated. |
...
**TOC load order:** `icons.lua`, `core.lua`, `data/spellterms.lua`, `scan.lua`, `aliases.lua`, `cleanup.lua`. `core.lua` is first because it calls `LibNAddOn(...)`; `scan.lua`/`aliases.lua`/`cleanup.lua` make load-time `ns:registerEvent`/`ns:registerCommand` calls that need the wired namespace (`data/spellterms.lua` only sets a field, so its position is free). All cross-module reads happen at runtime via `ns`, so beyond "setup first" the order is not otherwise significant.
...
mergedSpellTerms    = { [fileID] = " name1 name2 " }  -- in-memory union: live stores + bundled

-- data/spellterms.lua (shipped baseline; folded under live scans by rebuildMerged)
ns.bundledSpellTerms = { bySpec = { [specID] = {...} }, byRace = { [raceID] = {...} } }
...
rebuildMerged()           -- union live spec+race sections + ns.bundledSpellTerms into mergedSpellTerms
...
mergeBuckets/emitBuckets  -- /bmi export helpers: merge live+bundled per bucket, serialize sorted Lua
