...
| `spec/` | `busted` unit tests (run from the repo root via `.busted`). `spec/bmi.lua` is the shared loader — stubs WoW globals and runs a file the way WoW does, `assert(loadfile(f))("BetterMacroIcons", ns)`; `spec/dataset_spec.lua` covers `/bmi export` + `/bmi diff`. Pure logic only; frame/`C_*` code stays in-game-tested. |
...
- **`scan.lua` is removable.** Everything about spell terms lives there behind `ns.SpellTermsFor`; `core.lua` reads it guarded. Its DB sub-tables (`spellTermsBySpec`/`spellTermsByRace`/`spellTermsByClass`) are lazy-seeded inside `scan.lua` and untouched by `MigrateDB`, so deleting the file leaves no dangling reference — the orphaned data just lingers in SavedVariables.
